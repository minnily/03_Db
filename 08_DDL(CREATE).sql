...
-- 가입일 -▶ SYSDATE를 활용
INSERT INTO "MEMBER" VALUES 
('MEM02', 'QWER1234', '김영희', '970123-2234567', SYSDATE);
...
VALUES('MEM03', '10RSD24D', '이지연');



-- ** JDBC 에서 날짜를 입력 받았을 때 삽입하는 방법 **
-- '2022-09-19 17:33:27' 이런식의 문자열이 넘어온 경우 ...

INSERT INTO MEMBER VALUES
('MEM04','PASS04','김길동','930303-1234511',
...
--------▶ 0으로 시작할 가능성이 있으면 CHAR, VACHAR2 같은 문자형을 사용
...
- 데이터 무결성(=정확하고 일관성상태)  보장을 목적으로 함. -▶ 중복데이터 X
...
SELECT * FROM USER_CONSTRAINTS; --DBEAVER에서 사용하는 방법
...
-- 삽입/ 수정 시 NULL 값을 허용하지 않도록 컬럼레벨에서 제한하는 것
...
VALUES(1,'USER01','PSAA01','홍길동','남자','010-1234-2456','hong123@kh.or.kr');

INSERT INTO USER_USED_NN
VALUES(NULL,'USER01','PSAA01','홍길동','남자','010-1234-2456','hong123@kh.or.kr');
-- SQL Error [1400] [23000]:
-- ORA-01400: NULL을 ("KH_LMK"."USER_USED_NN"."USER_NO") 안에 삽입할 수 없습니다  ▶ 오류발생
...
INSERT INTO USER_USED_UK 
VALUES(1, 'USER01','PASS01', '홍길동', '남', '010-1234-5678', 'hong123@kh.or.kr');
-- ORA-00001: 무결성 제약 조건(KH_LMK.USER_ID_U)에 위배됩니다  ▶ 오류발생
...
-- ORA-00001: 무결성 제약 조건(KH_LMK.USER_ID_NAME_U)에 위배됩니다 ▶ 오류발생

 SELECT * FROM USER_USED_UK2;
-----------------------------------------------------------------------------------

-- 제약 조건 3. PRIMARY KEY(기본키) 제약조건
-- 테이블에서 한 행의 정보를 찾기위해 사용할 컬럼을 의미함.
-- 테이블에 대한 식별자(사용자번호, 학번 ...) 역할을 함

-- NOT NULL + UNIQUE 제약조건의 의미 --▶ 중복되지 않는 값이 필수로 존재해야 함.(기본조건)

-- 1) 한 테이블당 한 개만 설정할 수 있음!
--▶ EX) USER_NO NUMBER PRIMARY KEY,
--      USER_ID VARCHAR2(20) PRIMARY KEY  ▶ 불가능!!!!!
-- 2) 컬럼레벨, 테이블레벨 둘다 설정 가능함
-- 3) 한 개 컬럼에 설정할 수 있고, 여러개의 컬럼을 묶어서 설정할 수 있음.
--▶ EX) CONSTRAINT USER_ID_NAME_U PRIMARY KEY(USER_ID, USER_NAME) ▶ 가능!!!!

------▶ 1),3)에 둘다 사용을 할 수 없음 한테이블 당 한개만 설정할 수 있기 때문에 !
...
VALUES(1, 'USER02','PASS02', '이순신', '남', '010-2345-9987', 'sunsin005@kh.or.kr');

INSERT INTO USER_USED_PK
VALUES(1, 'USER01','PASS01', '홍길동', '남', '010-1234-5678', 'hong123@kh.or.kr');
-- 기본키 중복 오류 ▼
-- ORA-00001: 무결성 제약 조건(KH_LMK.USER_NO_PK)에 위배됩니다.

INSERT INTO USER_USED_PK
VALUES(NULL, 'USER03','PASS03', '유관순', '여', '010-9873-9877', 'sunny5@kh.or.kr');
-- 기본키 NULL 이므로 오류 ▼
-- ORA-01400: NULL을 ("KH_LMK"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다.


----------------------------------------------------------------------------------

-- PRIMARY KEY 복합키 (테이블 레벨에서만 설정가능)
...
-- ORA-00001: 무결성 제약 조건(KH_LMK.PK_USERNO_USERID)에 위배됩니다
-- 회원번호와 아이디 둘다 중복되었을때만 제약조건 위배 에러 발생!

INSERT INTO USER_USED_PK2
VALUES(NULL, 'USER01','PASS02', '신사임당', '여', '010-1234-5678', 'hong123@kh.or.kr');
-- ORA-01400: NULL을 ("KH_LMK"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다
...
-- 제약 조건 4. FOREIGN KEY( 외래키/외부키 ) 제약조건
...
INSERT INTO USER_USED_FK
VALUES(2, 'USER02','PASS02', '이순신', '남', '010-5678-9999', 'leesu@kh.or.kr', 10);

INSERT INTO USER_USED_FK
VALUES(3, 'USER03','PASS03', '유관순', '여', '010-4567-1234', 'yookk00@kh.or.kr', 30);

INSERT INTO USER_USED_FK
VALUES(4, 'USER04','PASS04', '안중근', '남', '010-2222-1111', 'an_middle7@kh.or.kr',null);
--▶ NULL 사용 가능

INSERT INTO USER_USED_FK
VALUES(5, 'USER05','PASS05', '윤봉길', '남', '010-6666-7864', 'YOUN_bong8@kh.or.kr',50);
-- ORA-02291: 무결성 제약조건(KH_LMK.GRADE_CODE_FK)이 위배되었습니다- 부모 키가 없습니다
--▶ 50이라는 값은 USER_GRADE 테이블 GRADE_CODE 컬럼에서 제공하는 값이 아니므로 외래키 제약조건에 위배되어 오류 발생
...
-- 01) ON DELETE RESTRICTED(삭제 제한)로 기본 지정되어 있음
-- FOREIGN KEY로 지정된 컬럼에서 사용되고 있는 값일 경우
-- 제공하는 컬럼의 값은 삭제하지 못함

DELETE FROM USER_GRADE WHERE GRADE_CODE = 30; -- 30은 자식테이블에서 사용되고 있는 값
-- ORA-02292: 무결성 제약조건(KH_LMK.GRADE_CODE_FK)이 위배되었습니다- 자식 레코드가 발견되었습니다
...
-- ON DELETE SET NULL 삭제옵션이 적용된 테이블 생성 (자식테이블)
...
-- 제약조건 5. CHECK 제약조건 : 컬럼에 기록되는 값에 조건 설정을 할 수 있음
-- CHECK ( 컬럼명 비교연산자 비교값 ) --▶ 범위도 사용가능함 
...
-- '남성' 이라는 조건 이외의 값이 들어와 에러발생
--ORA-02290: 체크 제약조건(KH_LMK.GENDER_CHECK)이 위배되었습니다.

...
--						ADD [CONSTRAINT 제약조건명] 제약조건 (지정할 컬럼명) [REFERENCES 테이블명 [(컬럼명)]]; <- FK(FOREIGN KEY) 인경우 추가
...
-- LNAME의 기본값을 'KOREA'로 수정정
ALTER TABLE DEPT_COPY MODIFY LNAME DEFAULT 'KOREA';
SELECT * FROM DEPT_COPY; 
--> 기본 값을 변경했다고 해서 기존 데이터 가 변하지는 않음
...
-- 						테이블에 데이터가 저장됨.
-- 테이블은 최소 1개 이상의 컬럼이 존재해야 되기 때문에 모든 컬럼을 다 삭제할 수는 없다.
...
-- 1) 컬럼명 변경 (DEPT_TITLE -> DEPT_ NAME)
...
-->DDL 이 종료된 후 DML구문을 수행할 수 있도록 권장!
